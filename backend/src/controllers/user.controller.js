...
export async function sendFriendRequest(req,res) {
    try {
       ...
       if(recipient.friends.includes(myId)){
        return res.status(400).json({message:"Your already frinds with user"});
       }
       ...
       res.status(201).json(friendRequest);
        ...
        console.error("error in sedn freind request controller");
        res.status(500).json({message:"internal server Error"});
    }
}
...
export async function acceptFriendRequest(req,res) {
    try {
        ...
        //add each user to the othrts friend array
        ...
        console.log("Error in accepted frind request controller",error.message);
        res.status(500).json({message:"Internal Server Error"});
    }
}
